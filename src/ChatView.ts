...
export class ChatView extends ItemView {
  ...
  private async sendMessage(): Promise<void> {
    ...
    if (this.activeFileContent && this.activeFilePath) {
      apiMessages.push({
        role: "system",
        content: `You are responding via an Obsidian sidebar chat plugin (HTTP API).\n\nThe user is currently viewing/editing this file in Obsidian:\n\nFile: ${this.activeFilePath}\n\nFile content:\n\`\`\`\n${this.activeFileContent}\n\`\`\`\n\nIMPORTANT RULES:\n1. Unless the user explicitly specifies a different file, ALL tasks target THIS file.\n2. For SIMPLE requests (questions, short edits, summaries): respond directly with the content.\n3. For COMPLEX requests (rewriting large sections, full outlines, multi-step tasks): First respond with a brief plan of what you will do, then provide the content. Break large outputs into sections.\n4. ALWAYS respond with text in your reply. Write suggested content directly — the user will copy or apply it. Do NOT use tools to edit files.\n5. If the user says "write here" or "add a paragraph", compose the text in your reply.\n6. This is a synchronous HTTP request — prioritize returning a response quickly over perfection.`,
      });
    }
